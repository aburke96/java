...
{
	private BigCat[] myCats;
	private int numCats;

	public BigCatListImpl()
	{
		this.numCats = 0;
		this.myCats = new BigCat[MAX_SIZE];
	}
	...
	public boolean add(BigCat bigCat)
	{
		...
		int loc = this.find(bigCat);
		
		if (loc != -1)
		{
			return false;
		}
		myCats[numCats] = bigCat;
		numCats++;
		return true;
	}
	...
	{
		BigCat retValue = null;
		if (position >= 0 && position < this.numCats)
		{
			retValue = this.myCats[position];
		}
		return retValue;
	}
